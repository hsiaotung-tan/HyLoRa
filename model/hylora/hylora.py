...
class CAB_lowrank(nn.Module):      
    def __init__(self, num_feat, squeeze_factor=30, memory_blocks=128):         
        ...
        self.cab = nn.Sequential(                        
            Prompt(num_feat, squeeze_factor, memory_blocks) 
        )      
...
class QKV_lowrank(nn.Module):
    def __init__(self, dim, qkv_bias, low_rank, memory_blocks, weight_factor, *args, **kwargs) -> None:
        ...
        self.v_lowrank = CAB_lowrank(dim, squeeze_factor=low_rank, memory_blocks=memory_blocks)
        self.weight_factor = weight_factor
        
    def forward(self, x):
        qk = self.qk(x)
        v = self.v(x)
        v_lowrank = self.weight_factor * self.v_lowrank(x) + v
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.qkv_lowrank = QKV_lowrank(dim, qkv_bias=qkv_bias, low_rank=down_rank, memory_blocks=memory_blocks, weight_factor=weight_factor)
